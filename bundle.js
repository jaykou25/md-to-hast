...
function decodeNamedCharacterReference(value) {
  return own$b.call(characterEntities, value) ? characterEntities[value] : false;
}
...
function createState(tree, options) {
  ...
  function oneBound(node, parent) {
    ...
    return one$3(state, node, parent);
  }
...
function one$3(state, node, parent) {
  ...
  if (own$6.call(state.handlers, type)) {
...
function all$4(state, parent) {
  ...
  if ('children' in parent) {
    ...
    while (++index < nodes.length) {
      const result = one$3(state, nodes[index], parent);
...
function defaultUnknownHandler(state, node) {
  ...
  const result = 'value' in node && !(own$6.call(data, 'hProperties') || own$6.call(data, 'hChildren')) ? {
    type: 'text',
    value: node.value
  } : {
    type: 'element',
...
function all$3(state, nodes) {
  ...
  while (++index < nodes.length) {
    ...
    result[index] = one$2(state, nodes[index]);
  }
  return result;
}
...
function element$2(state, node) {
  ...
  const result = fn(node.tagName, props, all$3(state, node.childNodes));
...
function toParse5(tree, options) {
  ...
  return one$1(tree, space === 'svg' ? svg : html);
}
...
function raw(tree, options) {
  ...
  /** @type {State} */
  const state = {
    parser: document ? new Parser(parseOptions) : Parser.getFragmentParser(null, parseOptions),
    handle(node) {
      one(node, state);
    },
    stitches: false,
...
function setPoint(state, point) {
  if (point.line && point.column && point.offset !== null && point.offset !== undefined) {
    /** @type {Location} */
    const location = {
...
function cloneWithoutChildren(node) {
  return 'children' in node ? extend$1(true, {}, {
    ...node,
    children: []
  }) : extend$1(true, {}, node);
}
